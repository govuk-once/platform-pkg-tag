...
import { DataClassificationTag } from '../src/tag/data-classification.tag';
import { ExposureTag } from '../src/tag/exposure.tag';
import { PIITag } from '../src/tag/pii.tag';
import { UtilTag } from '../src/tag/util.tag';
import { createTestApp, createTestStack, createLambda, VALID_PROPS } from './helpers/cdk.helpers';
...
describe('GovUKTag', () => {
  ...
  describe('fluent getters', () => {
    it('DataClassification returns DataClassificationTag', () => {
      const stack = createTestStack();
      const fn = createLambda(stack);
      expect(GovUKTag.of(fn).DataClassification).toBeInstanceOf(DataClassificationTag);
    });
  ...
  describe('buriedOf', () => {
    ...
    it('throws when path is ambiguous', () => {
      const stack = createTestStack();
      const parent1 = new Construct(stack, 'GroupA');
      createLambda(stack.node.findChild('GroupA') as any, 'Handler');
      const parent2 = new Construct(stack, 'GroupB');
      createLambda(stack.node.findChild('GroupB') as any, 'Handler');

      expect(() => GovUKTag.buriedOf(stack, 'Handler')).toThrow('Ambiguous path');
    });
  });
