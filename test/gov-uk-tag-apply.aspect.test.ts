...
describe('GovUKTagApplyAspect', () => {
  describe('visit', () => {
    ...
    it('applies optional tags when provided', () => {
      ...
      const tags = getTagValues(cfnFn);
      expect(tags['RepositoryUrl']).toBe('https://github.com/govuk-once/test-repo');
    ...
    it('skips optional tags that are undefined', () => {
      ...
      const aspect = new GovUKTagApplyAspect({
        ...VALID_PROPS,
        optionalAppTags: { RepositoryUrl: undefined, BillingProject: 'test-billing' },
      });
      aspect.visit(cfnFn);
    ...
    it('is a no-op when disabled', () => {
      ...
      const aspect = new GovUKTagApplyAspect({ ...VALID_PROPS, disabled: true });
      aspect.visit(cfnFn);
