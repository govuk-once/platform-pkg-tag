...
describe('RESOURCE_CATEGORIES', () => {
  ...
  it('every profile has a non-empty label', () => {
    for (const [key, profile] of entries) {
      ...
      expect(profile.label.length, `${key} label should not be empty`).toBeGreaterThan(0);
    }
  });
  ...
  it('every profile has boolean flags', () => {
    for (const [key, profile] of entries) {
      ...
      expect(typeof profile.dataClassificationRequired, `${key}.dataClassificationRequired`).toBe('boolean');
      expect(typeof profile.exposureRequired, `${key}.exposureRequired`).toBe('boolean');
    }
  });
  ...
  it('all cfnTypes follow AWS format', () => {
    for (const [key, profile] of entries) {
      for (const cfnType of profile.cfnTypes) {
        expect(
          cfnType,
          `${key} has malformed cfnType '${cfnType}'`,
        ).toMatch(/^AWS::\w+::\w+$/);
      }
    }
  });
