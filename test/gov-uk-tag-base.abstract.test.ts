...
import { GovUKTagApplyAspect } from '../src/gov-uk-tag-apply.aspect';
import { GovUKTagAspectBase } from '../src/gov-uk-tag-base.abstract';
import { GovUKEnvironments } from '../src/gov-uk-tag.const';
import { createTestStack, VALID_PROPS, VALID_MANDATORY_TAGS } from './helpers/cdk.helpers';

import type { GovUKTagAspectProps } from '../src/gov-uk-tag.types';
...
describe('GovUKTagAspectBase', () => {
  describe('validateTagValue', () => {
    ...
    it('rejects non-string values at runtime', () => {
      expect(() =>
        createAspect({
          mandatoryAppTags: { ...VALID_MANDATORY_TAGS, Product: 123 as unknown as string },
        }),
  ...
  describe('validateMandatoryAppTags', () => {
    ...
    it('throws when a mandatory tag is missing', () => {
      const { Product, ...rest } = VALID_MANDATORY_TAGS;
      expect(() =>
        createAspect({
  ...
  describe('validateOptionalAppTags', () => {
    ...
    it('passes with a valid RepositoryUrl', () => {
      expect(() =>
        createAspect({
          ...VALID_PROPS,
          optionalAppTags: { RepositoryUrl: 'https://github.com/govuk-once/flex' },
        }),
      ).not.toThrow();
    });
    ...
    it('rejects RepositoryUrl with trailing segments', () => {
      expect(() =>
        createAspect({
          ...VALID_PROPS,
          optionalAppTags: { RepositoryUrl: 'https://github.com/org/repo/tree/main' },
        }),
      ).toThrow('not a GitHub repository URL');
    });
    ...
    it('allows undefined optional tags', () => {
      expect(() =>
        createAspect({
          ...VALID_PROPS,
          optionalAppTags: { RepositoryUrl: undefined, BillingProject: undefined },
        }),
      ).not.toThrow();
    });
  ...
  describe('cfnTypeToResource', () => {
    it('maps AWS::Lambda::Function correctly', () => {
      const result = (GovUKTagAspectBase as unknown as { cfnTypeToResource: Map<string, string> })
        .cfnTypeToResource;
  ...
  describe('tagManagerOf', () => {
    it('returns tag manager for a taggable resource', () => {
      ...
      const tagManager = (GovUKTagAspectBase as unknown as { tagManagerOf: (node: CfnResource) => unknown })
        .tagManagerOf(cfnFn);
